...
class pluto_ecm_hw_command_processor_thread:
  def __init__(self, arg):
    self.logger = pluto_ecm_logger.pluto_ecm_logger(arg["log_dir"], "pluto_ecm_hw_command_processor_thread", arg["log_level"])
...
def pluto_ecm_hw_command_processor_thread_func(arg):
  thread = pluto_ecm_hw_command_processor_thread(arg)
  try:
    thread.run()
...
class pluto_ecm_hw_config:
  ...
  def send_reset(self):
    ...
    packed_data = PACKED_ECM_CONFIG_CONTROL.pack(ECM_CONTROL_MAGIC_NUM,
                                              self.seq_num,
                                              0,
                                              ECM_MODULE_ID_CONTROL, ECM_CONTROL_MESSAGE_TYPE_ENABLE,
                                              0, 0, 0, 1)
    self.seq_num += 1
    return self._send_data(packed_data, False)

  def send_enables(self, chan_enable, synth_enable, status_enable):
    self.logger.log(self.logger.LL_DEBUG, "[hw_cfg] send_enables: {} {} {}".format(chan_enable, synth_enable, status_enable))
    packed_data = PACKED_ECM_CONFIG_CONTROL.pack(ECM_CONTROL_MAGIC_NUM,
                                              self.seq_num,
                                              0,
                                              ECM_MODULE_ID_CONTROL, ECM_CONTROL_MESSAGE_TYPE_ENABLE,
                                              status_enable, synth_enable, chan_enable, 0)
    self.seq_num += 1
    return self._send_data(packed_data, False)
  ...
  def send_module_data(self, mod_id, msg_type, addr, data, expect_ack):
    ...
    packed_header = PACKED_ECM_CONFIG_HEADER.pack(ECM_CONTROL_MAGIC_NUM, self.seq_num, addr, msg_type, mod_id)
    self.seq_num += 1
