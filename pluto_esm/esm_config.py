...
class esm_config:
  PACKED_ESM_CONFIG_HEADER  = struct.Struct("<" + PACKED_UINT32 + PACKED_UINT32 + "xx" + PACKED_UINT8 + PACKED_UINT8)
  PACKED_ESM_CONFIG_CONTROL = struct.Struct("<" + PACKED_UINT32 + PACKED_UINT32 + "xx" + PACKED_UINT8 + PACKED_UINT8 +
  ...
  def send_module_data(self, mod_id, msg_type, data):
    packed_header = self.PACKED_ESM_CONFIG_HEADER.pack(ESM_CONTROL_MAGIC_NUM, self.seq_num, mod_id, msg_type)
    self.seq_num += 1
